...
export class SalesforceLeadPage extends PlaywrightWrapper {
     ...
     */
    async appLauncher() {
        await this.click(selectors.lead.appLauncher, 'App Launcher', 'Button');
    }
    ...
    async searchApp(value: string) {
        await this.type(selectors.lead.searchAppsInput, 'Search apps and items...', value);
    }

    /**
     * Clicks on the specified app name.
     * @param appName The name of the app to click.
     */
    async clickApp(appName: string) {
        await this.click(selectors.lead.appLink(appName), appName, 'App link');
    }
     ...
     * @param salutation The salutation to select.
     */
    ...
    /**
     * Verifies that the lead name is visible on the page.
     * @param fullName The full name to verify.
     */
    async verifyLeadName(fullName: string) {
        await this.spinnerDisappear();
        await expect(this.page.locator(selectors.lead.leadNameText(fullName))).toBeVisible();
    }
}
